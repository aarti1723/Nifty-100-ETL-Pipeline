...
def dq02_company_year(df):
    return df[df.duplicated(subset=["company_id", "year"])]
...
if __name__ == "__main__":
    ...
    pk_failures = dq01_primary_key(pnl, "id")
    company_year_failures = dq02_company_year(pnl)
    fk_failures = dq03_foreign_key(pnl, companies)
    sales_failures = dq06_positive_sales(pnl)
    bs_failures = dq04_balance_sheet(bs)
    opm_failures = dq05_opm_check(pnl)

    print("PK failures:", len(pk_failures))
    print("Company-Year failures:", len(company_year_failures))
    print("FK failures:", len(fk_failures))
    print("Sales failures:", len(sales_failures))
    print("Balance Sheet failures:", len(bs_failures))
    print("OPM failures:", len(opm_failures))
